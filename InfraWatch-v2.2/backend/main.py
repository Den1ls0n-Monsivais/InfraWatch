...
def agent_heartbeat(uid: str, data: HeartbeatSchema, db: Session = Depends(get_db)):
    ...
    if data.cpu_percent > 90:  _new_alert(db,a.id,"cpu","critical",f"{a.hostname}: CPU {data.cpu_percent:.0f}%")
    elif data.cpu_percent > 75:_new_alert(db,a.id,"cpu","warning", f"{a.hostname}: CPU {data.cpu_percent:.0f}%")
    if data.ram_percent > 90:  _new_alert(db,a.id,"ram","critical",f"{a.hostname}: RAM {data.ram_percent:.0f}%")
    if data.disk_percent > 90: _new_alert(db,a.id,"disk","critical",f"{a.hostname}: Disco {data.disk_percent:.0f}%")
    db.commit()
    return {"status": "ok"}
